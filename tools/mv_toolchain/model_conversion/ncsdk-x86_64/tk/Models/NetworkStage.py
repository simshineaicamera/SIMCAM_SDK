...
class NetworkStage:
    ...
    def __init__(
        ...
        elif self.op in [StageType.deconvolution, StageType.depthwise_deconvolution]:
            if pad_type == PadStyle.tfsame:
        ...
        if post_op_type and post_op_type != StageType.none:
            ...
            if (self.op in [StageType.convolution,
                            ...
                            StageType.fully_connected_layer]) and bias is not None:
    ...
    def addBias(self, bias, biasName=None):
        if bias is not None:
    ...
    def putBias(self):
        if self.bias is not None:
            self.biasPointer, self.biasBufferIndex = get_buffer(
                self.bias.astype(np.float16), self.datatype, self.biasName)
            self.biasIndex = MemoryIndex.blob.value

    def putTaps(self):
        if self.taps is not None:
            self.tapsPointer, self.tapsBufferIndex = get_buffer(
                self.taps.astype(np.float16), self.datatype, self.tapsName)
    ...
    def check_algo(self, network):
        ...
        # Force im2col in this case, where it's normally never used
        if self.op == StageType.convolution and self.inputDimZ >= 200:
            if network.writes_output(self, self.outputIndex):
                if self.optMask & 0x80000000 == 0x80000000:
                    self.optMask = (self.optMask & 0x7fffffff) | self._OPT_IM2COL
                    print(
                        'Layer ',
                        self.unprocessed_name,
                        ' forced to im2col, because its output is used in concat')
        for t in self.tail:
