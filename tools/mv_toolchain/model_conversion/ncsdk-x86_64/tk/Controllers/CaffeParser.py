        ...
        for prevlayer in top:
            if isReLU(layer.type):
                ...
                if layer.relu_param.negative_slope == 0.0:
                    ...
                    if(applicable_node.op == StageType.scale):
                        applicable_node.opParams[0] = 2#we try to merge scale and relu
                        applicable_node.postOp = StageType.none
                    elif applicable_node.op == StageType.eltwise_sum:
                        if (applicable_node.opParams is None) or applicable_node.opParams[0] == 0:
                            applicable_node.opParams[0] = 2#we try to merge relu eltwise_sum
                            applicable_node.postOp = StageType.none
                        elif applicable_node.opParams[0] == 1:
                            applicable_node.opParams[0] = 3#we try to merge relu and scale
                            applicable_node.postOp = StageType.none
                else:
                    applicable_node.postOp = StageType.leaky_relu
                    if(applicable_node.op == StageType.scale):
                        applicable_node.opParams[0] = 0
...
def parse_caffe(arguments, myriad_conf, debug=False, file_gen=False):
    ...
    for idx, layer in enumerate(layers):
        ...
        if isBatchNorm(layer.type) or isScale(layer.type):
            ...
            if node != 0 and (
                    node.op == StageType.convolution or node.op == StageType.depthwise_convolution or node.op == StageType.fully_connected_layer):
                ...
                if layer.name == outputNodeName:
                    break
                continue
            elif node != 0 and node.op == StageType.scale:
                w, b = get_caffe_params(layer, net.params)
                if (w.shape == node.taps.shape):
                    node.taps = node.taps * w
                    if node.bias is not None:
                        if b is not None:
                            node.bias = node.bias * w + b
                        else:
                    ...
                    if layer.name == outputNodeName:
                        break
                    continue
            elif node != 0 and node.op == StageType.eltwise_sum:
                w, b = get_caffe_params(layer, net.params)
                channel_num = w.shape[0]
                if (node.opParams is not None) and channel_num >= 8:
                    if node.opParams[0] == 0:
                        if b is not None:
                            merged_taps_to_bias = np.concatenate([w, b])
                        else:
                            b = np.zeros(w.shape)
                        node.addBias(np.array(merged_taps_to_bias).astype(np.float16))
                        node.postOp = StageType.none
                        node.opParams[0] = 1
                        node.name = node.unprocessed_name + '/' + layer.name
                        node.changeName(node.name)
                        node.alias.append(node.unprocessed_name)
                        ...
                        if layer.name == outputNodeName:
                            break
                        continue
                    elif node.opParams[0] == 1:# the previous layer is already elementwise-add+scale
                        old_tap_bias = node.bias
                        if old_tap_bias is not None and old_tap_bias.shape[0] == channel_num * 2:
                            old_taps = old_tap_bias[0 : channel_num]
                            old_bias = old_tap_bias[channel_num : ]
                            new_taps = old_taps * w
                            new_bias = old_bias * w + b
                            new_tap_bias = np.concatenate([new_taps, new_bias])
                            node.bias = new_tap_bias
                            node.name = node.unprocessed_name + '/' + layer.name
                            node.changeName(node.name)
                            node.alias.append(node.unprocessed_name)
                            node.alias.append(layer.name)
        ...
        if isCrop(layer.type):
            ...
            crop_offset = np.array([0, 0, 0], dtype=np.dtype("<i4"))
            ...
            crop_offset = np.array([crop_offset[2], crop_offset[1],
                                    crop_offset[0]], dtype=np.dtype("<i4"))
        ...
        if (isConcat(layer.type) or (isConvolution(layer.type) and get_caffe_kernel_size(layer)[0] > 1) or (
            ...
            if (len(prev_output_shape) == 1):
                ori_prev_shape0 = prev_output_shape[0]
                for slice in curslicing:
                    prev_output_shape = [[slice[2] - slice[1], ori_prev_shape0[1], ori_prev_shape0[2]]]
            for slice in curslicing:
                for i in range(len(top)):
                    if top[i] == slice[0]:
        ...
        if (arguments.explicit_concat and isConcat(layer.type) and len(top) <= 3) or (isConcat(layer.type) and len(top) == 1):
        ...
        if isDepthwiseConvolution(layer, get_caffe_output_channels(
            ...
            if layer.name == outputNodeName:
                break
            continue
        if isDepthwiseDeconvolution(layer, get_caffe_output_channels(
                layer, inshape, top, network), inshape[0]):
            ...
            taps = taps[:,:,::-1,::-1]
            depthwise_deconv_node = NetworkStage(layer.name, top,
                                          StorageOrder.orderZYX,  # s_order,
                                          # pad_x, pad_y, pad_type,
                                          get_caffe_op_padding(layer)[0],
                                          get_caffe_op_padding(layer)[1],
                                          ...
                                          DataType.fp16,
                                          # op_type
                                          StageType.depthwise_deconvolution,
                                          # op_x, op_y
                                          get_caffe_op_radix(layer)[0],
                                          get_caffe_op_radix(layer)[1],
                                          # sx, sy
                                          ...
                                          myriad_conf,
                                          args=arguments)
            network.attach(depthwise_deconv_node)
            last_layer = layer
            if layer.name == outputNodeName:
                break
            continue
        if isReorg(layer.type):
            reorg_param = np.array([(layer.reorg_param.stride)],
                    dtype=np.dtype("<i4"))
            reorg_node = NetworkStage(layer.name,
                                      top,
                                      StorageOrder.orderZYX,
                                      0,
                                      0,
                                      ...
                                      myriad_conf,
                                      args=arguments,
                                      opParams = reorg_param)

            network.attach(reorg_node)
        ...
        if (
            ...
            if(isConvolution(layer.type) or isDeconvolution(layer.type)):
                ...
                layer_params = np.array([conv_dilation], dtype=np.dtype("<i4"))
            elif(isScale(layer.type) or isBatchNorm(layer.type)):
                #by default, we set scale opt to be 1, indicating merging scale and bias together
                layer_params = np.array([1], dtype=np.dtype("<i4"))
            elif(isEltwise(layer.type)):
                #by default we set element-wise op-param to be 0, meaning no merge opt is required
                layer_params = np.array([0], dtype=np.dtype("<i4"))
    ...
    if outputNodeName is not None:
        if inputNodeName is not None:
            ...
            try:
                ...
                net.blobs['input_1'].data[...] = input_data
            try:
