...
def get_buffer(for_data, datatype, buffer_name=None):
    """
    ...
    if buffer_name in buffer_dict:
        return buffer_dict[buffer_name]
    ...
    if buffer_name is not None:
        buffer_dict[buffer_name] = (data_offset,len(buffer))
...
def replace_buffer(new_data, offset, datatype):
    ...
    # print(offset)
    if offset not in convert_flag:
        buffer[offset] = new_data
        convert_flag.add(offset)
