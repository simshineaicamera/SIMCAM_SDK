        ...
        for idx, node in enumerate(graph.get_operations()):
                ...
                if pattern_found:
                    if current_pattern.get_type() == PatternType.Completed:
                        ...
                        if current_pattern.get_type() == PatternType.LeakyReLU:
                            ...
                            if(prev_node.op == StageType.scale):
                                prev_node.opParams[0] = 0
            ...
            if node.type == "Const":
                ...
                if bias.size == 1:
                    ...
                    prev_node.addBias( (np.ones(outputs) * bias).astype(np.float16), bias_data.name)
                else:
                    prev_node.addBias( bias.astype(np.float16), bias_data.name)
                prev_node_label = strip_tensor_id(node.outputs[0].name)
                ...
                if(prev_node.op == StageType.scale and prev_node.opParams is not None):
                    prev_node.opParams[0] = 2#we try to merge scale and relu
                    prev_node.postOp = StageType.none
                prev_node_label = strip_tensor_id(node.outputs[0].name)
                prev_node.changeName(prev_node_label)
                ...
                inputs = node.inputs
                input_shape = node.inputs[0].get_shape()
                if strip_tensor_id(inputs[1].name) in shape_dict.keys():
                    desired_shape = shape_dict[strip_tensor_id(inputs[1].name)]
                else:
...
#                    desired_shape = np.reshape(input_shape.as_list(), desired_shape)[0]
                ...
                shape_dict[node.name] = node.inputs[0].get_shape()
                ...
                explicit = False
                #If one op has already been used once in concat, we have to use explicit concat this time
                if (node.type == 'Concat'):
                    input_indices = range(1, len(node.inputs))
                else:
                    input_indices = range(0, len(node.inputs) - 1)
                for idx in input_indices:
                    curr_input_name = strip_tensor_id(node.inputs[idx].name)
                    for concat_entry in concat_tracker:
                        for l in concat_entry[1]:
                            if (l == curr_input_name):
                                explicit = True
                                break;
                        if (explicit == True):
                            break;
                ...
                if explicit or arguments.explicit_concat:
                    ...
                    if node.type == 'Concat':
                        ...
                        input_indices = range(0, len(node.inputs) - 1)
                    if (axis_select == 0 and len(node.inputs[1].get_shape()) == 2):
                        axis_select = 1
                    outstride = 0
                    for idx in input_indices:
                        if (axis_select == 0 and len(node.inputs[idx].get_shape()) == 2):
                            outstride += int(node.inputs[idx].get_shape()[0]) * int(node.inputs[idx].get_shape()[1])
                        else:
                    ...
                    for idx in input_indices:
                        ...
                        prev_node = NetworkStage(strip_tensor_id(node.outputs[0].name) + '_' + str(idx), [in_node] if in_node!=0 and in_node is not None else None, StorageOrder.orderYXZ,
                            ...
                            1, 1,  # op_x, op_y,
                            1, 1,  # sx, sy,
                            int(inshape[2]), int(inshape[1]), int(inshape[3]),  # X, Y, Z
                            0, 0, int(inshape[3]),
                        ...
                        if idx == 0:
                            ...
                            if node.name == output_node_name:
                                outputPointer, outputIndex = prev_node.setoutput(
                        ...
                        node_dict[strip_tensor_id(node.outputs[0].name) + '_' + str(idx)] = prev_node
                        outputPointer = outputPointer + 2 * int(inshape[3])
                        cnt += 1
                        addednames.append(strip_tensor_id(node.outputs[0].name) + '_' + str(idx))
                    prev_node = firstnode
                    if node.name == output_node_name:
                        firstnode.isoutput = True
                        break
                    else:
                        concat_tracker += [(prev_node_label, addednames)]
                    prev_op = node
                    prev_op = node
                    continue
                if debug:
                ...
                cnt += 1
            elif (node.type == 'FusedBatchNorm' and len(node.inputs) == 5):
                if debug:
                    print('FusedBatchNorm')

                # Fold the batchnorm into the weights
                if prev_node_label is not None and (prev_node.op == StageType.convolution or \
                   prev_node.op == StageType.fully_connected_layer or \
                   prev_node.op == StageType.depthwise_convolution) and \
                   (prev_op is not None and prev_op.type != "ConcatV2" and prev_op.type != "Concat"):
                    if debug:
                    ...
                    top = get_input(strip_tensor_id(inputs.name))
                    input_shape = node.inputs[0].get_shape()
                    scaleParams = np.array([1], dtype=np.dtype("<i4"))
                    if len(input_shape) == 4:
                        xyz = (int(input_shape[2]),
                               int(input_shape[1]),
                               int(input_shape[3]))
                        ...
                        xyz = (1, 1, int(input_shape[1]))
                    scale = np.reshape(scale, [xyz[2]])
                    bias= np.reshape(bias, [xyz[2]])
                    prev_node = NetworkStage(
                        # Name, Top, Order
                        node.name,
                        top,
                        ...
                        PadStyle.none,
                        DataType.fp16, DataType.fp16,
                        # Op, radixX, radixY StrideX, StrideY
                        StageType.scale,
                        1,
                        1,
                        1,
        ...
        if len(res.shape) == 4:
            ...
            network.outputTensor = (1,
                res.shape[0],
                res.shape[1],
                res.shape[2])
        elif len(res.shape) == 2:
            network.outputTensor = (1, 1,
                res.shape[0],
                res.shape[1])
        else:
